...
import { IProject, ISession } from './project.model'
...
export class ProjectService {
  ...
  getPojects(): Observable<IProject[]> {
    return this.http.get('/api/projects').map((response: Response) => {
      return <IProject[]>response.json();
    }).catch(this.handleExpection)
  }
  getProjects() {
    return projects
  }
  ...
  getProjectAsync(id: number): Observable<IProject> {
     return this.http.get('/api/projects'+ id).map((response: Response) => {
      return <IProject>response.json();
    }).catch(this.handleExpection)
  }
  saveProject(project) {
  ...
  saveProjectAsyn(project) : Observable<IProject>{
    let headers = new Headers({'Content-Type:':'application/json'})
    let requestOptions = new RequestOptions({headers: headers});
    return this.http.post('/api/events',project,Option).map((response: Response)=>{
  ...
  searchSessions(searchTerm: string) {
    ...
    projects.forEach(project => {
      var matchedSessions = project.sessions.filter(session =>
  ...
  searchSessionAysn(searchTerm : string){
      return this.http.get('/api/sessions/search?search='+ searchTerm).map((response: Response) => {
      return response.json();
    }).catch(this.handleExpection)
  }
  handleExpection(error: Response) {
    console.log('adsdsdsad  ')
